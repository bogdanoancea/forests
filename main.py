...
INSERT INTO protests (
    source_file, source_name, date_raw, date_iso, time_raw, author, link, title, text, tags, content_hash
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
        ...
        cur.executemany(
            ...
            [
                (
                    ...
                    r.author, r.link, r.title, r.text, r.tags, r.content_hash
                )
                for r in records
            ],
        )
...
def main():
    ...
    if all_records:
        ...
        for rec, tfidf_tags, btag, lda_tags, hdb_tags, umap_hdb_tags in zip(
            ...
            combined = []
            if(tfidf_tags):
                combined.append(f"TFIDF: {tfidf_tags}")
            if btag:
                combined.append(f"bert: {btag}")
            if lda_tags:
                combined.append(f"LDA: {lda_tags}")
            if(hdb_tags):
                combined.append(f"HDBSCAN: {hdb_tags}")
            if(umap_hdb_tags):
                combined.extend(f"EMBED_HDBSCAN: {umap_hdb_tags}")
